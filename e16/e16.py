...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
        ...
        for time in range(200):
            ...
            #Reward bigger velocities
            reward += abs(speed) * 400
            ...
            #Punish for staying in the bottom of the hill at low speed, this
            #1. Gets the car moving faster at start
            #2. Maximizes the speed at the bottom of the hill meaning better velocity
            if(position > -0.55 and position < -0.45 and abs(speed) < 0.005):
                reward -= 3
                
            #Have some punishment for taking too long
            reward -= (time) * 0.005
            if(position > 0.5):
                reward = 600
                
            #Big reward for achiving goal
            if(term == True):
                reward = 700
            ...
            if(not TRAIN or True):
                print(f"rew:{reward:.3f} speed:{absspeed:.5f} pos:{position:.3f}")
            ...
            if (done):
                ...
                if TRAIN:
                    agent.replay(len(agent.memory), score)
                if SAVE:
