...
#    env = gym.make('MountainCar-v0')
    ...
    if LOAD:
        agent.load("weights-300-backup2.h5")
    done = False
    batch = 300
    ...
    for e in range(EPISODES):
        ...
        if score > best_score:
            best_score = score
            with open(file_path, 'w') as file:
                # Write the new integer to the file as a string
                file.write(str(best_score))
                print(f"new best score {best_score} saving file")
